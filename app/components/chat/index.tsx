...
const Chat: FC<IChatProps> = ({
  ...
  useEffect(() => {
    // Scroll to the bottom with smooth behavior whenever messages change
    if (chatContainerRef.current) {
      chatContainerRef.current.scrollTo({
        top: chatContainerRef.current.scrollHeight,
        behavior: 'smooth',
  ...
  return (
    <div className={cn(!feedbackDisabled && 'px-0', 'mr-0px', 'h-full')}>
      ...
      <div ref={chatContainerRef}
        className={`${s.customScrollbar} pl-[20px] pr-[20px] overflow-x-hidden overflow-y-auto  h-full space-y-[30px] `}>
        {chatList.map((item) => {
          if (item.isAnswer) {
            const isLast = item.id === chatList[chatList.length - 1].id
            return <div className={`${s.ans} w-[90%] md:w-[70%] `}>
              <Answer
                key={item.id}
                item={item}
